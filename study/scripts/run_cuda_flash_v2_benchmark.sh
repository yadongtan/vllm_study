...
result_stamp=${1:?usage: run_cuda_flash_v2_benchmark.sh YYYYMMDDHHMM}
result_dir="study/cuda_benchmark_results/$result_stamp"
mkdir -p "$result_dir"
concurrencies=(1 4 8)
port=8012
server_log="$result_dir/cuda-flash-v2-server.log"
server_pid=""
monitor_pid=""
...
STUDY_USE_CUDA_FLASH_ATTENTION_V2=1 \
...
.venv/bin/python3 study/scripts/resource_monitor.py monitor \
  --output "$result_dir/resources-baseline-cuda-flash-v2-loaded.csv" \
  --duration 5

for concurrency in "${concurrencies[@]}"; do
  .venv/bin/python3 study/scripts/resource_monitor.py monitor \
    --output "$result_dir/resources-cuda-flash-v2-concurrency-$concurrency.csv" &
  monitor_pid=$!
  ...
  .venv/bin/vllm bench serve --backend openai \
    ...
    --result-filename "cuda-flash-v2-concurrency-$concurrency.json"
